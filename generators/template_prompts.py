...
[⚠️ 분량 가이드 - 최우선 규칙! 이 규칙을 어기면 글이 발행되지 않습니다]

🔴 절대 최소 분량: {min_words}자 이상 (공백 포함)
🔴 목표 분량: {min_words}자 ~ {max_words}자
🔴 {min_words}자 미만이면 품질 미달로 자동 폐기됩니다!

분량 체크리스트:
- 서론: 최소 300자
- 본문 각 섹션: 최소 400자 (소제목당)
- 소제목: 최소 5개 이상
- 마무리: 최소 200자
- 합산: 반드시 {min_words}자 이상

분량을 채우는 방법:
1. 각 섹션별로 충분한 설명과 구체적인 예시 포함
2. 독자가 궁금해할 추가 정보 제공
3. 실제 사례나 통계 데이터 인용 (구체적 수치 필수)
4. "왜?"와 "어떻게?"에 대한 깊이 있는 답변
5. 관련된 실용적인 정보 추가
6. 각 항목마다 3~5문장 이상 상세 설명
7. 비교표, 체크리스트, FAQ 등 부가 콘텐츠 추가

❌ 하지 말 것:
- 300자짜리 짧은 글 출력 (절대 금지!)
- 같은 내용 반복하여 분량 늘리기
- 의미 없는 문장으로 채우기
...
[이미지 태그 형식 - 매우 중요!]
- 반드시 [IMAGE_1], [IMAGE_2] 형식으로만 작성 (최대 2개)
